## Ваш текст с исправленными вставками изображений
...
1. Теперь скачиваем Outline Manager и устанавливаем (выбирайте под свою систему — [ссылка](https://developers.google.com/outline/docs/download-links?hl=ru)):
   ...
   2. Вставляем ключ, который мы получили ранее по команде выше (зелёный) (скрин 5 куда вставлять)
      [скрин 5](1963.jpg)
   3. После чего вы увидите меню вашего сервера. Там вы можете добавлять ключи, менять их название, смотреть сколько трафика сожрали и т.п.
   4. Сейчас нас там интересует пункт "Настройки". Заходим, и в поле "Порт для новых ключей доступа" вписываем порт 5349 (скрин 6, 7)
      [скрин 6](1965.jpg)
      [скрин 7](1966.jpg)
   5. После этого возвращаемся на вкладку "ключи доступа" и удаляем все имеющиеся там ключи через меню (3 точки справа от них)
   6. Далее создаём необходимое количество ключей через кнопку "Добавить новый ключ" и именуем каждый, чтобы далее не запутаться (Создавайте их по схеме 1 ключ = 1 человек. Ключ можно использовать как на компьютере, так и на телефоне, поэтому создавать несколько на 1 человека смысла нет)
   7. Почти последний пункт, чтобы поделиться ключом, нажимаем на значок "поделиться" (возле 3 точек находится) (скрин 8)
      [скрин 8](1977.jpg)
   8. И финальный шаг. Берём скопированный ключ `ss://Y2hhY2hhMXXXXXXXXXXXXXXXXXXXXXXXXXX@217.147.XX.XXX:5349/?outline=1`, и добавляем к его концу префикс для маскировки трафика `&prefix=%16%03%01%00%C2%A8%01%01`.
