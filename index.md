...
## 1. Выбор сервера.
   Я использую сервер от [https://vdska.ru/](https://vdska.ru/). Там можно посмотреть тарифы и выбрать подходящую локацию. **Ограничение трафика почти на каждой локации — 3ТБ в месяц**
   ...
   *Для надёжности, советую в последствии купить ещё один сервер в другой локации, и настроить там тоже впн. Если вдруг возникнут проблемы с одним, легко можно будет переключиться на второй*
...
//командой выше мы получим ключ для подключения к нашему менеджеру outline. Он будет выделен зелёным (скрин ниже)
```
(скриншот 5 как выглядит ключ)
![скрин 5](1980.jpg)

```
//Ниже ключа указаны 2 порта (скрин ниже), доступ к которым нужно открыть. И возле каждого порта указан протокол UDP, TCP или UDP+TCP. Вводим команды:
sudo ufw allow номер_порта1/tcp
sudo ufw allow номер_порта2/tcp
sudo ufw allow номер_порта2/udp

```
(скриншот 6. Порты, которые нужно открыть)
![скрин 6](1979.jpg)

## 3. Настройка Outline Manager:

   1. [Скачиваем](https://developers.google.com/outline/docs/download-links?hl=ru) и устанавливаем Outline Manager.
   2. Нажимаем на добавление сервера (скрин 7)
      ![скрин 7](1962.jpg)
   3. Вставляем ключ, который мы получили ранее по команде выше (зелёный) (скрин 8 куда вставлять)
      ![скрин 8](1963.jpg)
   ...
   5. Сейчас нас там интересует пункт "Настройки". Заходим, и в поле "Порт для новых ключей доступа" вписываем порт 5349 (скрин 9, 10)
      ![скрин 9](1965.jpg)
      ![скрин 10](1966.jpg)
   6. Теперь возвращаемся в наш Power Shell, и добавляем доступ к порту 5349:
   ```
   sudo ufw allow 5349/tcp
   sudo ufw allow 5349/udp
   ```
   7. Возвращаемся в Outline Manager, на вкладку "Ключи доступа", и удаляем все имеющиеся там ключи через меню (3 точки справа от них)
   8. Создаём необходимое количество ключей через кнопку "Добавить новый ключ" и именуем каждый, чтобы в будущем не запутаться (Создавайте их по схеме 1 ключ = 1 человек. Ключ можно использовать как на компьютере, так и на телефоне, поэтому создавать несколько на 1 человека смысла нет)
   9. Почти последний пункт. Чтобы поделиться ключом, нажимаем на значок "поделиться" (возле 3 точек находится) (скрин 11)
      ![скрин 11](1977.jpg)
   10. И финальный шаг. Берём скопированный ключ `ss://Y2hhY2hhMXXXXXXXXXXXXXXXXXXXXXXXXXX@217.147.XX.XXX:5349/?outline=1`, и добавляем к его концу префикс для маскировки трафика `&prefix=%16%03%01%00%C2%A8%01%01`.
      То есть итоговый вариант рабочего ключа = ключ + префикс: `ss://Y2hhY2hhMXXXXXXXXXXXXXXXXXXXXXXXXXX@217.147.XX.XXX:5349/?outline=1&prefix=%16%03%01%00%C2%A8%01%01`
