...
```
//Ниже ключа указаны 2 порта (скрин ниже), доступ к которым нужно открыть. И возле каждого указан порта указан протокол UDP, TCP или UDP+TCP. Вводим команды:
...
## 3. Настройка Outline Manager:
   ...
   ```
   7. После этого возвращаемся на вкладку "Ключи доступа" и удаляем все имеющиеся там ключи через меню (3 точки справа от них)
